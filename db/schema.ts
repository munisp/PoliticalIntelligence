...
export const budgets = mysqlTable(
  "budgets",
  {
    ...
    /**
     * Budget tier from the ingestion connectors:
     * federal | state | faac_allocation | budget_execution |
     * development_partner (nullable for legacy/seed rows).
     */
...
export const POLICY_ALERT_SOURCES = ["bill", "regulation", "budget"] as const;

/**
 * Radar alerts generated by the deterministic impact rubric
 * (contracts/radar.ts). alertId is a natural key
 * ("alert:<sourceEntity>:<sourceRef>") so weekly scans are idempotent.
 */
export const policyAlerts = mysqlTable(
  "policy_alerts",
  {
    id: serial("id").primaryKey(),
    ...
    jurisdictionId: varchar("jurisdiction_id", { length: 64 }),
    sector: varchar("sector", { length: 64 }).notNull(),
    sourceEntity: mysqlEnum("source_entity", POLICY_ALERT_SOURCES).notNull(),
    /** laws.law_id / policy_documents.document_id / budgets.budget_id. */
    sourceRef: varchar("source_ref", { length: 255 }).notNull(),
    title: varchar("title", { length: 512 }).notNull(),
    summary: text("summary"),
    /** Deterministic rubric score in [0,100]. */
    impactScore: double("impact_score").notNull(),
    /** [{stakeholderId, name, kind}] — stakeholders whose tags match. */
    matchedStakeholders: json("matched_stakeholders"),
    createdAt: timestamp("created_at").defaultNow().notNull(),
    origin: mysqlEnum("origin", ["live", "derived", "seed"])
      .default("derived")
      .notNull(),
  ...
  (t) => ({
    sectorIdx: index("policy_alerts_sector_idx").on(t.sector),
    jurIdx: index("policy_alerts_jur_idx").on(t.jurisdictionId),
  }),
);

export type PolicyAlert = typeof policyAlerts.$inferSelect;
export type InsertPolicyAlert = typeof policyAlerts.$inferInsert;

/* ------------------------------------------------------------------ */
/* I3 — Corridor Twin (delivery milestones)                            */
/* ------------------------------------------------------------------ */

export const CORRIDOR_MILESTONE_STATUSES = [
  "planned",
  "in_progress",
  "done",
  "delayed",
...
/**
 * Corridor delivery milestones (docs/INNOVATIONS.md §I3). milestoneId is a
 * natural key ("ms:lch-...") so the Lagos–Calabar seed stays idempotent;
 * dates are ISO labels (YYYY-MM-DD) consistent with officials.validFrom.
 */
export const corridorMilestones = mysqlTable(
  "corridor_milestones",
  {
    id: serial("id").primaryKey(),
    /** Natural key, e.g. "ms:lch-s1-commissioning". */
    milestoneId: varchar("milestone_id", { length: 96 }).notNull().unique(),
    /** Corridor identifier, e.g. "corridor:lagos-calabar". */
    corridorId: varchar("corridor_id", { length: 96 }).notNull(),
    title: varchar("title", { length: 255 }).notNull(),
    plannedDate: varchar("planned_date", { length: 32 }).notNull(),
    actualDate: varchar("actual_date", { length: 32 }),
    status: mysqlEnum("status", CORRIDOR_MILESTONE_STATUSES)
      .default("planned")
      .notNull(),
    pctComplete: double("pct_complete").default(0).notNull(),
    /** Cumulative disbursement against this milestone, in ₦. */
    fundingDisbursedNgn: double("funding_disbursed_ngn"),
    /** evidence_sources.evidence_source_id or a free-form ref. */
  ...
  (t) => ({
    corridorIdx: index("corridor_milestones_corridor_idx").on(t.corridorId),
  }),
);

export type CorridorMilestone = typeof corridorMilestones.$inferSelect;
export type InsertCorridorMilestone = typeof corridorMilestones.$inferInsert;
...
/**
 * Engagement log: each row records one analyst↔stakeholder touchpoint.
 * Own-user scoped (a user sees only their own log); stakeholderId and
 * userId reference the natural key / serial respectively.
 */
export const stakeholderEngagements = mysqlTable(
  "stakeholder_engagements",
  {
    id: serial("id").primaryKey(),
    /** stakeholders.stakeholder_id (natural key). */
    stakeholderId: varchar("stakeholder_id", { length: 96 }).notNull(),
    /** users.id (serial) of the analyst who logged the engagement. */
    userId: bigint("user_id", { mode: "number", unsigned: true }).notNull(),
    engagedAt: timestamp("engaged_at").notNull(),
    /** e.g. "meeting" | "call" | "email" | "roundtable" | "site_visit". */
    channel: varchar("channel", { length: 32 }).notNull(),
    outcome: text("outcome"),
    commitments: text("commitments"),
    nextAction: text("next_action"),
    nextActionDate: varchar("next_action_date", { length: 32 }),
    createdAt: timestamp("created_at").defaultNow().notNull(),
  },
  (t) => ({
    stkIdx: index("stakeholder_engagements_stk_idx").on(t.stakeholderId),
    userIdx: index("stakeholder_engagements_user_idx").on(t.userId),
  }),
);

export type StakeholderEngagement = typeof stakeholderEngagements.$inferSelect;
export type InsertStakeholderEngagement =
  typeof stakeholderEngagements.$inferInsert;
/* ADDITIVE (feat-innov-6-10, I6): Public participation — bill comments */
/* Anonymous commenting allowed (nullable userId + pseudonym); bodies  */
/* are PII-redacted at the API boundary before persistence.            */
/* ------------------------------------------------------------------ */
...
export const billComments = mysqlTable(
  "bill_comments",
  {
    id: serial("id").primaryKey(),
    /** Natural key, e.g. "cmt:abc123". */
    commentId: varchar("comment_id", { length: 96 }).notNull().unique(),
    /** laws.law_id the comment is attached to. */
    lawId: varchar("law_id", { length: 64 }).notNull(),
    /** users.id — null for anonymous comments. */
    userId: bigint("user_id", { mode: "number", unsigned: true }),
    /** Display name for anonymous commenters (PII-redacted on input). */
    pseudonym: varchar("pseudonym", { length: 128 }),
    body: text("body").notNull(),
    sentimentHint: mysqlEnum("sentiment_hint", COMMENT_SENTIMENTS)
      .default("neutral")
      .notNull(),
    /** string[] — deterministic keyword-bucket themes. */
    themeTags: json("theme_tags"),
    status: mysqlEnum("status", COMMENT_STATUSES)
      .default("visible")
      .notNull(),
    createdAt: timestamp("created_at").defaultNow().notNull(),
  },
  (t) => ({
    lawIdx: index("bill_comments_law_idx").on(t.lawId),
    statusIdx: index("bill_comments_status_idx").on(t.status),
  }),
);

export type BillComment = typeof billComments.$inferSelect;
export type InsertBillComment = typeof billComments.$inferInsert;

/* ------------------------------------------------------------------ */
/* ADDITIVE (feat-innov-6-10, I7): State domestication tracker          */
/* Federal law × (36 states + FCT) domestication status grid.          */
/* ------------------------------------------------------------------ */
...
export const domesticationStatus = mysqlTable(
  "domestication_status",
  {
    id: serial("id").primaryKey(),
    /** Federal law reference (laws.law_id or a citation-style ref). */
    lawRef: varchar("law_ref", { length: 128 }).notNull(),
    /** 2-letter postal code or slug, e.g. "LA", "KD", "FCT". */
    state: varchar("state", { length: 32 }).notNull(),
    status: mysqlEnum("status", DOMESTICATION_STATUSES)
      .default("not_started")
      .notNull(),
    /** State assembly bill reference, when one exists. */
    billRef: varchar("bill_ref", { length: 128 }),
    /** Evidence pointer (gazette / assembly record URI). */
  ...
  (t) => ({
    lawState: uniqueIndex("domestication_law_state").on(t.lawRef, t.state),
    stateIdx: index("domestication_state_idx").on(t.state),
  }),
);

export type DomesticationRow = typeof domesticationStatus.$inferSelect;
export type InsertDomesticationRow = typeof domesticationStatus.$inferInsert;

/* ------------------------------------------------------------------ */
/* ADDITIVE (feat-innov-6-10, I9): Scenario marketplace                 */
/* Published scenario runs (with reproducibility hashes) that peer      */
/* jurisdictions can fork and verify.                                   */
/* ------------------------------------------------------------------ */

export const publishedScenarios = mysqlTable(
  "published_scenarios",
  {
    id: serial("id").primaryKey(),
    /** Natural key, e.g. "pub:abc123". */
    publishedId: varchar("published_id", { length: 96 }).notNull().unique(),
    /** simulation_runs.simulation_run_id being shared. */
    scenarioRunId: varchar("scenario_run_id", { length: 64 }).notNull(),
    /** users.id of the publisher. */
    publishedBy: bigint("published_by", { mode: "number", unsigned: true }),
    title: varchar("title", { length: 255 }).notNull(),
    summary: text("summary"),
    forkCount: int("fork_count").default(0).notNull(),
    /** Snapshot of the run's reproducibility hash at publish time. */
    reproducibilityHash: varchar("reproducibility_hash", { length: 64 }),
    publishedAt: timestamp("published_at").defaultNow().notNull(),
  },
  (t) => ({
    runIdx: index("published_scenarios_run_idx").on(t.scenarioRunId),
  }),
);

export type PublishedScenario = typeof publishedScenarios.$inferSelect;
export type InsertPublishedScenario = typeof publishedScenarios.$inferInsert;

/* ------------------------------------------------------------------ */
/* ADDITIVE (feat-innov-6-10, I10): Field verification loop             */
/* GPS-stamped field_officer verifications for milestones, projects     */
/* and outcome metrics; ≥2 confirmed upgrades verification_status.      */
/* ------------------------------------------------------------------ */
...
export const fieldVerifications = mysqlTable(
  "field_verifications",
  {
    id: serial("id").primaryKey(),
    /** Natural key, e.g. "fv:abc123". */
    verificationId: varchar("verification_id", { length: 96 }).notNull().unique(),
    entityType: varchar("entity_type", { length: 32 }).notNull(),
    /** Entity reference, e.g. "series:12:2025-06" or a project id. */
    entityRef: varchar("entity_ref", { length: 255 }).notNull(),
    /** users.id of the field officer. */
    verifierId: bigint("verifier_id", { mode: "number", unsigned: true }).notNull(),
    gpsLat: double("gps_lat").notNull(),
    gpsLng: double("gps_lng").notNull(),
    photoUri: varchar("photo_uri", { length: 512 }),
    verdict: mysqlEnum("verdict", FIELD_VERDICTS).notNull(),
    notes: text("notes"),
    createdAt: timestamp("created_at").defaultNow().notNull(),
  },
  (t) => ({
    entityIdx: index("field_verifications_entity_idx").on(
      t.entityType,
      t.entityRef,
    ),
  }),
...
export type InsertFieldVerification = typeof fieldVerifications.$inferInsert;
