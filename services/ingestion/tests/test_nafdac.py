"""nafdac connector tests — recorded fixtures only, NO network access."""
...
FIXTURE = "nafdac_guidelines.json"


def test_fixture_fallback_triggers_when_source_unreachable():
    conn = NafdacConnector(client=offline_client())
...
def test_provenance_stamping_derived_origin():
    ...
    assert prov.source_id == "nafdac_regulations"
...
def test_live_path_uses_source_url_and_live_origin():
    ...
    conn = NafdacConnector(client=mock_client({"/regulations": fixture}))
    raw = conn.fetch("jur:ng", None, {})
    assert raw[0].provenance.origin == "live"
    assert raw[0].provenance.url == "https://www.nafdac.gov.ng/regulations"
...
def test_registry_loader_and_scheduler_wiring():
    assert REGISTRY["nafdac"] is NafdacConnector
    assert get_connector("nafdac").name == "nafdac"
    assert ENTITY_KEYS["bill_document"] == "policy_documents"
    assert NafdacConnector.default_fixture().exists()
    from app.scheduler import DEFAULT_CADENCE_S
    assert DEFAULT_CADENCE_S["nafdac"] == 30 * 24 * 3600  # monthly
