"""nerc connector tests — recorded fixtures only, NO network access."""
...
FIXTURE = "nerc_orders.json"


def test_fixture_fallback_triggers_when_source_unreachable():
    conn = NercConnector(client=offline_client())
...
def test_provenance_stamping_derived_origin():
    ...
    assert prov.source_id == "nerc_regulations"
...
def test_quantitative_instrument_emits_sector_metric():
    ...
    assert metrics
    m = metrics[0].data
    assert m["metric_key"] == "metering_rate_pct"
    assert m["sector_code"] == "power"
    assert isinstance(m["value"], float)


def test_live_path_uses_source_url_and_live_origin():
    fixture = load(FIXTURE)
    conn = NercConnector(client=mock_client({"/regulations": fixture}))
    raw = conn.fetch("jur:ng", None, {})
    assert raw[0].provenance.origin == "live"
    assert raw[0].provenance.url == "https://nerc.gov.ng/regulations"
...
def test_registry_loader_and_scheduler_wiring():
    assert REGISTRY["nerc"] is NercConnector
    assert get_connector("nerc").name == "nerc"
    assert ENTITY_KEYS["bill_document"] == "policy_documents"
    assert NercConnector.default_fixture().exists()
    from app.scheduler import DEFAULT_CADENCE_S
    assert DEFAULT_CADENCE_S["nerc"] == 30 * 24 * 3600  # monthly
