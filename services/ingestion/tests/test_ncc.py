"""ncc connector tests — recorded fixtures only, NO network access."""
...
FIXTURE = "ncc_instruments.json"


def test_fixture_fallback_triggers_when_source_unreachable():
    conn = NccConnector(client=offline_client())
...
def test_provenance_stamping_derived_origin():
    ...
    assert prov.source_id == "ncc_regulations"
...
def test_quantitative_instrument_emits_sector_metric():
    ...
    assert metrics
    m = metrics[0].data
    assert m["metric_key"] == "broadband_penetration_pct"
    assert m["sector_code"] == "telecoms"
    assert isinstance(m["value"], float)


def test_live_path_uses_source_url_and_live_origin():
    fixture = load(FIXTURE)
    conn = NccConnector(client=mock_client({"/regulations": fixture}))
    raw = conn.fetch("jur:ng", None, {})
    assert raw[0].provenance.origin == "live"
    assert raw[0].provenance.url == "https://ncc.gov.ng/regulations"
...
def test_registry_loader_and_scheduler_wiring():
    assert REGISTRY["ncc"] is NccConnector
    assert get_connector("ncc").name == "ncc"
    assert ENTITY_KEYS["bill_document"] == "policy_documents"
    assert NccConnector.default_fixture().exists()
    from app.scheduler import DEFAULT_CADENCE_S
    assert DEFAULT_CADENCE_S["ncc"] == 30 * 24 * 3600  # monthly
