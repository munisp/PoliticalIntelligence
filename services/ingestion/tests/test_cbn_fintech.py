"""cbn_fintech connector tests — recorded fixtures only, NO network access."""
...
FIXTURE = "cbn_fintech_circulars.json"


def test_fixture_fallback_triggers_when_source_unreachable():
    conn = CbnFintechConnector(client=offline_client())
...
def test_provenance_stamping_derived_origin():
    ...
    assert prov.source_id == "cbn_fintech_circulars"
...
def test_quantitative_instrument_emits_sector_metric():
    ...
    assert metrics
    m = metrics[0].data
    assert m["metric_key"] == "licensed_psp_operators"
    assert m["sector_code"] == "fintech"
    assert isinstance(m["value"], float)


def test_live_path_uses_source_url_and_live_origin():
    fixture = load(FIXTURE)
    conn = CbnFintechConnector(client=mock_client({"/documents/circulars": fixture}))
    raw = conn.fetch("jur:ng", None, {})
    assert raw[0].provenance.origin == "live"
    assert raw[0].provenance.url == "https://www.cbn.gov.ng/documents/circulars"
...
def test_registry_loader_and_scheduler_wiring():
    assert REGISTRY["cbn_fintech"] is CbnFintechConnector
    assert get_connector("cbn_fintech").name == "cbn_fintech"
    assert ENTITY_KEYS["bill_document"] == "policy_documents"
    assert CbnFintechConnector.default_fixture().exists()
    from app.scheduler import DEFAULT_CADENCE_S
    assert DEFAULT_CADENCE_S["cbn_fintech"] == 30 * 24 * 3600  # monthly
