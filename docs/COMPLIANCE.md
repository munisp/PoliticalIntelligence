...
| SR-8 | Legislation workbench | ✅ | improved | `api/legislation.ts compare` (clause-level alignment reusing the innovations policyDiff engine via `api/lib/policy-diff.ts`), REST `POST /v1/legislation/compare`, `api/tests/v4-backend.test.ts` (determinism + parity with `innovations.policyDiff`) | Compare endpoint live; clauses still seeded, not parsed | Specialist analysis path |
...
| API-4 | Versioned REST routes under `/v1` | ✅ | improved | `api/rest.ts` mounted at `/v1` in `api/boot.ts` — now includes `GET /v1/auth/me`, `GET /v1/auth/permissions`, `GET /v1/sectors`, `GET /v1/briefs/{id}`, `POST /v1/legislation/compare`; contract tests in `api/tests/rest.test.ts` + `api/tests/v4-backend.test.ts` (129 vitest green) | `POST /v1/documents`, `/v1/admin/*`, `/v1/observability/*` still absent; brief create is `POST /v1/briefs` vs documented `POST /v1/briefs/generate` | Complete remaining route coverage or amend API.md |
| API-5 | Idempotency-Key on mutating POSTs | ✅ | improved | `api/rest.ts requireIdempotencyKey` enforced uniformly on all mutating REST POSTs (`/v1/opportunities/generate`, `/v1/scenarios`, `/v1/scenarios/:id/runs`, `/v1/briefs`) — 400 `IDEMPOTENCY_KEY_REQUIRED` error envelope when absent; tested in `api/tests/v4-backend.test.ts` | tRPC surface still uses a body field | — |
| API-6 | Health/readiness endpoint | ✅ | improved | `api/boot.ts` `GET /healthz` runs a real DB readiness probe (`SELECT 1` via `getDb`) → 200 `{status:"ok", db:"up"}` or 503 `{status:"degraded", db:"down"}`; verified by `api/tests/rest.test.ts`, `api/tests/v4-backend.test.ts` (up) and `api/tests/healthz-down.test.ts` (mocked down) | — | — |
...
| API-10 | Auth endpoints `GET /v1/auth/me`, `/permissions` | ✅ | improved | `api/auth-router.ts` (`me` + new `permissions` procedure over `ROLE_SCOPES` + `accessibleJurisdictionIds` from `api/utils/rbac.ts`), REST routes in `api/rest.ts`, scope/jurisdiction tests in `api/tests/v4-backend.test.ts` | — | — |
