...
| SEC-5 | Encryption in transit/at rest, Vault secrets | 🟡 | improved | `infra/k8s/base/cert-manager.yaml` (ClusterIssuer), `infra/k8s/base/external-secrets.yaml` (ESO SecretStore + ExternalSecrets), `infra/k8s/base/ingress.yaml` TLS stanza (`policy-twin-tls`), prod NetworkPolicy | Manifests landed but **not yet applied to a live staging cluster**; `example.gov.ng` host placeholders must be substituted per environment (see `docs/DEPLOY-CHECKLIST.md`); Vault itself still external | Apply to staging + substitute placeholders, then flip to ✅ |
